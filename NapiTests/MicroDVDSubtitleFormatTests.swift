...
class MicroDVDSubtitleFormatTests: XCTestCase {
    ...
    func testMicroDVDSubtitleFormatFormattedValue() {
        ...
        XCTAssertEqual(
            MicroDVDSubtitleFormat(startFrame: 0, stopFrame: 250, linesOfText: oneLineOfText).formattedString(),
            "{0}{250}Simple one line of text"
        )
        
        let multipleLinesOfText = ["This is a first line of text", "This is second", "And this is next one"]
        XCTAssertEqual(
            MicroDVDSubtitleFormat(startFrame: 250, stopFrame: 1500, linesOfText: multipleLinesOfText).formattedString(),
